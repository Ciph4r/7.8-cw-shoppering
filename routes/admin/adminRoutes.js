const router = require('express').Router()
const Category = require('./catagories/models/Catagory')
const {check , validationResult} = require('express-validator')
const checkCategory = require('./catagories/utils/checkCategory')
const {waterfall} = require('async')
const faker = require('faker')
const Product = require('./products/models/Products')
const addProduct = require('./products/helper/createProducts')
const categoryValidation = require('./adminValidation/categoryValidation')

router.get('/add-category' , (req,res) => {
    return res.render('admin/add-category')
})


///////////////////WORKING COPY

// router.post('/add-category' , checkCategory ,(req,res , next) => {
//     const errors = validationResult(req)
//     if(!errors.isEmpty()){
//         // return res.status(422).json({errors: errors.array()})
//         req.flash('errors', errors.errors[0].msg)
//         return res.redirect('/api/admin/add-category')
//     }

//         const category = new Category()
//         category.name = req.body.name

//         category.save().then((savedCategory) => {
            
//             return res.redirect(`/api/admin/create-product/${savedCategory.name}`)
    
//             // res.json({message: 'Success' , category: savedCategory})
//         })
//         .catch((err) => {
//             if(err.code === 11000){
//                 req.flash('errors' , 'Category already exist')
//                 return res.redirect('/api/admin/add-category')
//             }else{
//                 return next(err)
//             }
//         })
//     })


    // router.get('/create-product/:name' , (req,res) => {
    //     waterfall([
    //         callback => {
    //             Category.findOne({name: req.params.name}, (err , category) => {
    //                 if(err) return next(err)
    //                 callback(null , category)
    //             })
    //         },
    //         (category) => {
    //             for (let i = 0 ; i < 24 ; i++){
    //                 const product = new Product()
    //                 product.category = category._id
    //                 product.name = faker.commerce.productName()
    //                 product.price = faker.commerce.price()
    //                 product.image = `/images/products2/${i}.jpg`
    //                 product.description = faker.lorem.paragraph()
    //                 product.save().then((product) => console.log(product))
    //             }
    //         }
    //     ])
    //     req.flash('messages' , `Successfully added  ${req.params.name.toUpperCase()}`)
    //     return res.redirect('/api/admin/add-category')
    //     // return res.json({message: 'success'})
    // })

///////////////////WORKING COPY





    
router.post('/add-category' , [checkCategory, categoryValidation] ,(req,res , next) => {
        const category = new Category()
        category.name = req.body.name

        category.save().then((savedCategory) => {
                addProduct(savedCategory.name).then(()=> {
                    req.flash('messages' , `Successfully added  ${savedCategory.name.toUpperCase()}`)
                    return res.redirect('/api/admin/add-category')
                })
                .catch((err) => {
                    console.log(err)
                    req.flash('errors' , 'Unable to ADD')
                    return res.redirect('/api/admin/add-category')
                  })
            // res.json({message: 'Success' , category: savedCategory})
        })
        .catch((err) => {
            if(err.code === 11000){
                req.flash('errors' , 'Category already exist')
                return res.redirect('/api/admin/add-category')
            }else{
                return next(err)
            }
        })
    })



module.exports = router